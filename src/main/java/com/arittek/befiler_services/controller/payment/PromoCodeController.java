...
public class PromoCodeController {
    ...
    public ResponseEntity<Status> promoCode(@RequestBody PaymentCustomerInfoBean paymentCustomerInfoBean) throws Exception {
        ...
        try {
            ...
            if (!StringUtils.isNotEmpty(paymentCustomerInfoBean.getCustomer())) {
                return new ResponseEntity<>(new Status(6, "Please enter customer name."), HttpStatus.OK);
            }

            if (!StringUtils.isNotEmpty(paymentCustomerInfoBean.getMobileNo())) {
                return new ResponseEntity<>(new Status(6, "Please enter mobile no."), HttpStatus.OK);
            }

            if (!StringUtils.isNotEmpty(paymentCustomerInfoBean.getEmailAddress())) {
                return new ResponseEntity<>(new Status(6, "Please enter email address"), HttpStatus.OK);
            }

            if (!StringUtils.isNotEmpty(paymentCustomerInfoBean.getResidentialAddress())) {
                return new ResponseEntity<>(new Status(6, "Please enter residential address"), HttpStatus.OK);
            }

            if (!StringUtils.isNotEmpty(paymentCustomerInfoBean.getBillingAddress())) {
                return new ResponseEntity<>(new Status(6, "Please enter billing address"), HttpStatus.OK);
            }

            if (paymentCustomerInfoBean.getPaymentCartBeanList() == null || paymentCustomerInfoBean.getPaymentCartBeanList().size() < 0) {
                return new ResponseEntity<>(new Status(6, "Empty cart"), HttpStatus.OK);
            }

            if (paymentCustomerInfoBean.getUserId() != null) {
                user = usersServices.findOneByIdAndStatus(paymentCustomerInfoBean.getUserId(), UserStatus.ACTIVE);
                if (user == null) {
                    return new ResponseEntity<>(new Status(6, "Session Expired"), HttpStatus.OK);
                }
            } else {
                return new ResponseEntity<>(new Status(6, "Session Expired"), HttpStatus.OK);
            }

            if (!StringUtils.isNotEmpty(paymentCustomerInfoBean.getPromoCode())) {
                return new ResponseEntity<>(new Status(6, "Please enter Promo Code"), HttpStatus.OK);
            }

            PromoCode promoCode = promoCodeServices.findOneActiveRecordByPromoCode(paymentCustomerInfoBean.getPromoCode());
            if (promoCode == null) {
                return new ResponseEntity<>(new Status(6, "Promo Code expired"), HttpStatus.OK);
            }
            ...
            if (userAttemptsCount != null && userAttemptsCount.getPaymentAttemptsCount() != null) {
                if (settings != null && settings.getMaxPaymentAttempts() != null) {
                    if (userAttemptsCount.getPaymentAttemptsCount() >= settings.getMaxPaymentAttempts()) {
                        return new ResponseEntity<>(new Status(6, "Max attempts"), HttpStatus.OK);
                    }
                } else {
                        ...
                        if (paymentCart.getProductType() == ProductType.TAXFORM) {
                            if (paymentCart.getTaxform() != null && paymentCart.getSettingPayment() != null) {
                                ...
                                if (payment == null) {
                                    ...
                                    return new ResponseEntity<>(new Status(6, "Tax Form is already paid : " + paymentCart.getTaxform().getTaxformYear().getYear()), HttpStatus.OK);
                                }
                            }
                            ...
                            if (paymentCart.getFbrUserAccountInfo() != null && paymentCart.getSettingPayment() != null) {
                                ...
                                if (payment == null) {
                                    ...
                                    return new ResponseEntity<>(new Status(6, "NTN Registration is already paid : " + paymentCart.getTaxform().getTaxformYear().getYear()), HttpStatus.OK);
                                }
                            }
                        }
                    } else {
                        usersServices.increasePaymentCountByOne(user, "Register :: Incomplete Payment Cart");
                        return new ResponseEntity<>(new Status(6, "Payment cart error"), HttpStatus.OK);
                    }
                } else {
                    usersServices.increasePaymentCountByOne(user, "Register :: Incomplete Payment Cart");
                    return new ResponseEntity<>(new Status(6, "Payment cart error"), HttpStatus.OK);
            ...
            if (paymentCustomerInfo == null) {
                ...
                return new ResponseEntity<>(new Status(6, "Customer Info Error"), HttpStatus.OK);
            }
            ...
            return new ResponseEntity<>(new Status(7, "Successfull"), HttpStatus.OK);
        } catch (Exception e) {
            ...
            return new ResponseEntity<>(new Status(6, "Incomplete data"), HttpStatus.OK);
        }
    }
}
